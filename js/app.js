...
function createBoardingPassFallbackCanvas(el) {
  ...
  // ── Route ───────────────────────────────────────────────────────────────
  const ROUTE_Y = 268;  // ★ 268로 변경 (기존 248, +20px 여백 추가)
  const CODE_SIZE = 66, CITY_SIZE = 30;
  ...
  // ── Ticket tear divider ──────────────────────────────────────────────────
  const DIV_Y = 408;  // ★ 408로 변경 (기존 388, +20px)
  ...
  const FIELD_Y0 = 442;  // ★ 442로 변경 (기존 422, +20px)
  ...
  const contentH = FOOTER_Y + 70;  // 푸터 텍스트 끝 + 둥근 모서리 여백
  ctx.globalCompositeOperation = 'destination-in';
  roundRectPath(ctx, 0, 0, W, contentH, 56);
  ctx.fillStyle = 'black';
  ctx.fill();
  ctx.globalCompositeOperation = 'source-over';

  // ── 1080×1350 출력 (여백 없이 전체 채우기, 모서리 투명) ──────────────
  const outCanvas = document.createElement('canvas');
  outCanvas.width = 1080;
  outCanvas.height = 1350;
  const outCtx = outCanvas.getContext('2d');
  // 흰 배경 없음 → 모서리 투명 PNG 출력
  outCtx.drawImage(canvas, 0, 0, W, contentH, 0, 0, 1080, 1350);
  return outCanvas;
}
