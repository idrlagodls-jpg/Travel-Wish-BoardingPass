...
function go(step) { state.screen = 'booking'; state.step = step; syncPassengers(); if (step === 3) ensureSeatDefaults(); render(); window.scrollTo({ top: 0, behavior: 'smooth' }); }
...
function boardingPassFileName(item) {
  ...
  return `WishBoardingPass_v15-1_${booking}_${String(item.index + 1).padStart(2, '0')}_${sector}_${route}_${passenger}.png`;
}
...
function createBoardingPassFallbackCanvas(el) {
  ...
  if (hasMeal) {
    ...
    ctx.fillStyle = '#101828';
    drawFitText(ctx, `기내식 · ${meal.title}`, PAD, nextY + 28, W - PAD * 2, 900, 40, 28, 'left'); // 데이터 +6 (34→40)
    nextY += 86;
  }
...
function drawNoSpendSocialCard(ctx, x, y, w, h, compact=false) {
  ...
  // ── Title ─────────────────────────────────────────────────────────────────
  ctx.fillStyle = '#101828';
  setCanvasFont(ctx, 900, compact ? 22 : 28);
  ctx.textAlign = 'left';
  ctx.fillText('오늘의 무지출 발권 완료', x + pad, y + (compact ? 32 : 40));
  ctx.fillStyle = '#667085';
  setCanvasFont(ctx, 500, compact ? 14 : 16);
  drawFitText(ctx, '결제하지 않고, 언젠가의 여행 목표를 먼저 남겼어요.', x + pad, y + (compact ? 62 : 78), w - pad * 2, 500, compact ? 14 : 16, 12, 'left');

  // ── Airport codes (fixed 38px) ────────────────────────────────────────────
  const codeY = y + (compact ? 108 : 130);
  const CODE_SIZE = 38;
  const cityOffset = CODE_SIZE + 10; // gap between code baseline and city name

  ctx.fillStyle = '#101828';
  drawFitText(ctx, f.departure_airport_id, x + pad, codeY, compact ? 200 : 240, 900, CODE_SIZE, 28, 'left');
  ctx.fillStyle = '#101828';
  drawFitText(ctx, f.arrival_airport_id, x + w - pad, codeY, compact ? 200 : 240, 900, CODE_SIZE, 28, 'right');
  ctx.fillStyle = '#2563eb';
  drawFitText(ctx, '✈', x + w / 2, codeY + 8, 60, 900, compact ? 26 : 30, 20, 'center');

  ctx.fillStyle = '#667085';
  drawFitText(ctx, airportLabel(f.departure_airport_id), x + pad, codeY + cityOffset, compact ? 220 : 260, 500, compact ? 15 : 17, 12, 'left');
  drawFitText(ctx, airportLabel(f.arrival_airport_id), x + w - pad, codeY + cityOffset, compact ? 220 : 260, 500, compact ? 15 : 17, 12, 'right');
  ctx.textAlign = 'left';

  // Thin separator line before info rows
  const sepY = codeY + cityOffset + (compact ? 26 : 30);
  ctx.strokeStyle = '#e4e7ec';
  ctx.lineWidth = 1;
  ctx.beginPath();
  ...
  // ── Info rows ─────────────────────────────────────────────────────────────
  // KEY FIX: rowsRight = full card right edge (NOT limited by QR position)
  // QR is placed BELOW all rows, so there is no horizontal overlap.
  const fullRight = x + w - pad;          // card right edge (for values)
  const rowStart  = y + (compact ? 230 : 290);
  const rowGap    = compact ? 36 : 44;
  const rowFontSz = compact ? 17 : 20;
  ...
  drawFitText(ctx, returnText, x + pad, y + h - (compact ? 42 : 52), returnMaxW, 900, compact ? 15 : 19, 12, 'left');
}
function downloadSocialImage(kind) {
  ...
  try {
    ...
    triggerPngDownload(canvas, `WishBoardingPass_v15-1_${kind}_${safeFilePart(state.bookingCode || 'DEMO')}.png`);
  } catch (error) { alert(error.message || 'SNS 이미지 저장 중 오류가 발생했습니다.'); }
}
function startWishBoardingPass() { state.screen = 'booking'; state.step = 0; render(); window.scrollTo({ top:0, behavior:'smooth' }); }
function startTravelTest() { state.screen = 'quiz'; state.quizStep = 0; state.quizAnswers = {}; state.recommendation = null; render(); window.scrollTo({ top:0, behavior:'smooth' }); }
...
function renderPrivacyModal() {
  ...
  modalSheet.innerHTML = `
    ...
    <div class="privacy-body">
      ...
      <p>향후 서비스 개선을 위해 웹 분석 도구를 사용하는 경우, 처리 항목과 목적을 본 방침에 반영합니다.</p>
      ...
      <p style="color:#667085;font-size:12px;margin-top:16px">시행일자: 2026년 06월 28일</p>
    </div>`;
}
...
function renderComplete() {
  ...
  app.innerHTML = `
    <section class="card"><h2 class="section-title">여행 위시 저장 완료</h2><p class="muted">오늘은 결제하지 않고, 언젠가 떠날 여행 목표를 먼저 저장했어요. WISH/DEMO 표시가 들어간 보딩패스를 PNG로 공유할 수 있습니다.</p></section>
    <section class="card no-spend-card"><span class="chip orange">결제 대신 목표 저장</span><h2 class="section-title">오늘의 무지출 발권 완료</h2><p class="wish-save-copy">충동 결제 대신, 가고 싶은 여행을 하나의 위시로 남겼어요.</p><div class="summary-line"><span>실제 결제 금액</span><b>0원</b></div><div class="summary-line"><span>아낀 금액</span><b>${won(totalPrice())}</b></div><div class="summary-line"><span>대신 저장한 여행</span><b>${h(routeWishText())}</b></div><div class="summary-line"><span>저장한 이유</span><b>여행 목표 저장</b></div><div class="summary-line"><span>상태</span><b>언젠가 꼭 가기</b></div></section>
    <section class="card download-panel">
      <h2 class="section-title">보딩패스 PNG 저장</h2>
      <p class="section-caption">개별 저장은 보딩패스마다 1장씩 저장하고, 전체 저장은 현재 발급된 보딩패스 ${passCount}장을 각각 PNG 파일로 한 번에 저장합니다. SNS 이미지는 선택한 문구와 여행 위시 요약이 함께 저장됩니다.</p>
      <div class="field"><label>SNS 저장 문구</label>${shareMessageButtonsHtml()}<textarea class="share-textarea" oninput="updateShareMessage(this.value)">${h(state.shareMessage || DEFAULT_SHARE_MESSAGE)}</textarea></div>
      <button class="primary" onclick="downloadAllBoardingPassPng()">전체 보딩패스 저장</button>
      <div class="social-save-grid" style="margin-top:10px">
        <button class="secondary" onclick="downloadBoardingPassPng(0)">이 보딩패스 PNG 저장</button>
        <button class="secondary" onclick="downloadAllBoardingPassPng()">전체 보딩패스 저장</button>
        <button class="secondary" onclick="downloadSocialImage('story')">스토리용 1080×1902</button>
        <button class="secondary" onclick="downloadSocialImage('square')">정사각형 1080×1080</button>
      </div>
      <p class="notice">여러 장을 한 번에 저장할 때 브라우저가 여러 파일 다운로드 허용을 요청할 수 있습니다.</p>
    </section>
    ${renderShareSection()}
    ${boardingPassesHtml()}
    <div class="footer-actions"><button class="secondary" onclick="resetToHome()">다시 발권하기</button><button class="primary" onclick="downloadAllBoardingPassPng()">전체 보딩패스 저장</button></div>`;
}
...
function boardingPassHtml(f, sector, p, index = 0, fileName = '') {
  const seat = p.type === 'infant' ? 'INF' : (state.seats[sector]?.[p.id] || '-');
  const meal = p.type === 'infant' ? null : mealFor(sector, p.id);
  const mealBlock = mealAvailable() && meal && meal.id !== 'none' ? `<div style="grid-column:1/-1;border-top:1px dashed #e4e7ec;margin-top:4px;padding-top:14px"><label>Meal</label><b>${h(meal.title)}</b></div>` : '';
