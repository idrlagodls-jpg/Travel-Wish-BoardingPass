...
function startWishBoardingPass() {
  gtag('event', 'boarding_pass_start'); // ★ 핵심: 보딩패스 만들기 클릭 수
...
function completeBooking() {
  ...
  const primaryMeal = mealAvailable()
    ...
    : '없음';
  gtag('event', 'boarding_pass_issued', {
...
function copyShareLink() {
  gtag('event', 'share_link_copy'); // ★ 핵심: 공유하기(링크복사) 클릭 수
  const url = getSiteUrl();
...
function downloadFromDataUrl(imgEl) {
  // ★ 핵심: PNG 저장 수 / 분석: 저장 포맷별 클릭률
  gtag('event', 'save_png', { save_format: 'boarding_pass' });
